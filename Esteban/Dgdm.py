...
def INITIALIZE(frequencies, NumberOutputSymbols): #D = alphabet output
    assert(len(frequencies)>1)
    N = len(frequencies)
    D = NumberOutputSymbols
    n0 = (N - 2) % (D-1) + 2 # 2 <= n0 <= D

    externals = [ExternalNode(frequencies,i) for i in range(n0,len(frequencies))] # N - n0 external nodes
    internals = [InternalNode(frequencies, [ExternalNode(frequencies,i) for i in range(0, n0)])] #internal with n0 external nodes
